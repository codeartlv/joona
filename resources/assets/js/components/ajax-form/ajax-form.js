...
export default class AjaxForm {
	...
	constructor(formElement, settings) {
		...
		this.handler.bindEvent('loaded', (response) =>
			this.onSuccess(response)
		);
		this.handler.bindEvent('failed', (status, message) =>
		...
		if (this.settings.focus) {
			let field = this.form.querySelector(
				`[name="${this.settings.focus}"]`
			);
	...
	onSuccess(response) {
		...
		if (response.status == 'error') {
			...
			this.form.dispatchEvent(
				new CustomEvent('form.error', { detail: response })
			);
		}
		...
		if (response.status == 'success') {
			...
			this.form.dispatchEvent(
				new CustomEvent('form.success', { detail: response })
			);
		}
	}
	...
	findControl(name) {
		...
		if (inputControl) {
			messageContainer = this.form.querySelector(
